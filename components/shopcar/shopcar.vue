<template>
	<view>
			...
			<uni-swipe-action-item  class="swipeItem" v-for="(obj,index) in list" :options="obj.options" :key="obj.options[0].ID"  @click="swipeClick($event,index)">
				...
				<view class="contBox">
					<view class="circleBox" @click.stop="inp(obj.options[0].ID)">
						<image src="../../static/img/circleCachee.png"  class="circle"  v-if="!obj.options[0].check"></image>
						<image src="../../static/img/circleCacheef.png" class="circle"  v-if="obj.options[0].check"></image>
					</view>
					<view class="goodBox">
						<view>{{obj.options[0].name}}<text>{{obj.options[0].price}}</text></view>
						<view class="countBox">
							<view class="sign" @click="signCount(obj.options[0].ID)">
								<image src="../../static/img/signf.png"  v-if="obj.options[0].count>1"></image>
								<image src="../../static/img/signe.png"  v-if="obj.options[0].count==1"></image>
							</view>
							<view class="count">{{obj.options[0].count}}</view>
							<view class="add" @click="addCount(obj.options[0].ID)">
								<image src="../../static/img/addf.png"   v-if="obj.options[0].stock>obj.options[0].count"></image>
								<image src="../../static/img/adde.png"   v-if="obj.options[0].stock==obj.options[0].count"></image>
							</view>
						</view>
					</view>
				</view>
		...
		<view class="allBox">
			<view>
				<view @click="all()">
					<image src="../../static/img/circleCachee.png"  v-if="flag"></image>
					<image src="../../static/img/circleCacheef.png"   v-if="!flag"></image>
					<text>全选</text>
				</view>
